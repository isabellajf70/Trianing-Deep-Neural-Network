...
class NeuralNetwork(object):
    ...
    def diff_actFun(self, z, type):
        ...
        if type == 'Tanh':
            res = (2 / (1 + np.exp(-2 * z))) - 1
            res = (-2) * np.exp(-2 * z) * 2 * (-1) / np.square((1 + np.exp(-2 * z)))

        if type == 'Sigmoid':
            res = (1/(1 + np.exp(-z)))
            res = (-1) * np.exp(-z) * (-1) / np.square(1 + np.square(-z))
    ...
    def backprop(self, X, y):
        ...
        delta2 = (delta1 @ self.W2.T) * self.diff_actFun(self.z1, self.actFun_type)
        dW2 = np.transpose(self.a1) @ delta1
        db2 = np.sum(delta1, axis=0)
        dW1 = np.transpose(X) @ delta1
